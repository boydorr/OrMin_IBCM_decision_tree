...
DT <- decision_tree(N = N,
                    pop = pop * 3, HDR_min = HDR[1], HDR_max = HDR[2], # population x 3 because 3 years!
                    ...
                    confirmed_animal = RiskOverall$positive)
...
for (i in 1:nrow(RiskYr)) {
  ...
  DT_y <- decision_tree(N = N, 
                  ...
                  confirmed_animal = RiskYr$positive[i])
...
DT_yr_summary$names <- c("Year", "Recorded deaths", "Estimated deaths", "Estimated exposures", "Estimated exposures not given PEP",
              "Estimated exposure incidence per 100,000", "Laboratory confirmed animal cases", "Estimated rabid dogs",
              "Estimated % confirmed cases", "Estimated rabid dogs per 1000 dogs")
write.csv(DT_yr_summary, "outputs/Table4_decision_tree_by_year.csv", row.names = FALSE)
...
for (i in 1:nrow(RiskMun)) {
  # model
  DT_mun <- decision_tree(N = N, # population multiplied by 3 because over study years 2020-2022
                        pop = RiskMun$population * 3, HDR_min = HDR[1], HDR_max = HDR[2], 
                        pObsDeath = pObsDeath, pDeath = pDeath, mean_bites = mu, k_bites = k,
                        death_data = RiskMun$deaths[i], 
                        high_risk_min = RiskMun$high_risk_dogs[i], 
                        high_risk_max = RiskMun$high_risk[i] + RiskMun$unk_risk[i],
                        confirmed_animal = RiskMun$cases[i])
  ...
  DatFram <- data.frame(
    ...
    Est_dogs =  paste0(round(Q_mun$dog_population[2], digits = 0), "  [", 
                       round(Q_mun$dog_population[1], digits = 0), " - ", round(Q_mun$dog_population[3], digits = 0), "]"),
    ...
    Recorded_positives = RiskMun$cases[i], 
    ...
    Est_exposure_incidence = paste0(round(Q_mun$exposure_incidence[2], digits=0), "  [", 
                                    round(Q_mun$exposure_incidence[1], digits=0), " - ", round(Q_mun$exposure_incidence[3], digits=0), "]"))
  DT_df <- rbind(DT_df, DatFram) ## Add rows
}
DT_df

# Results Table 5 - Decision tree estimates by municipality with Q50 [Q025-Q975] formatting 
DT_mun_summary <- as_tibble(DT_df)
